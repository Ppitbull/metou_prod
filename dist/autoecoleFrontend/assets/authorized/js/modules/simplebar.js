...
export   class SimpleBar {
  constructor(element, options) {
    ...
    this.classNames = {
      ...this.defaultOptions.classNames,
    ...
    this.getRtlHelpers = memoize(this.getRtlHelpers);
   ...
   */
   getRtlHelpers() {
    ...
    const dummyContainerOffset = this.getOffset(scrollbarDummyEl);
    const dummyContainerChildOffset = this.getOffset(dummyContainerChild);
    scrollbarDummyEl.scrollLeft = 999;
    const dummyContainerScrollOffsetAfterScroll = this.getOffset(
      dummyContainerChild
    );
  ...
  init() {
    ...
    this.instances.set(this.el, this);
  ...
  positionScrollbar(axis = 'y') {
    ...
    scrollOffset =
      axis === 'x' &&
      this.isRtl &&
      this.getRtlHelpers().isRtlScrollingInverted
        ? -scrollOffset
        : scrollOffset;
    ...
    handleOffset =
      axis === 'x' &&
      this.isRtl &&
      this.getRtlHelpers().isRtlScrollbarInverted
  ...
  drag = e => {
    ...
    if (this.draggedAxis === 'x') {
      scrollPos =
        this.isRtl && this.getRtlHelpers().isRtlScrollbarInverted
          ? scrollPos - (trackSize + scrollbar.size)
          : scrollPos;
      scrollPos =
        this.isRtl && this.getRtlHelpers().isRtlScrollingInverted
          ? -scrollPos
          : scrollPos;
    }
